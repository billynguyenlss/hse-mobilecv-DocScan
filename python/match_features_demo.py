#copied from https://github.com/rpautrat/SuperPoint/blob/master/superpoint/match_features_demo.py
import argparse
...
def convert_to_tflite(models_dir):
    converter = tf.compat.v1.lite.TFLiteConverter.from_frozen_graph(
        graph_def_file=os.path.join(models_dir,'superpoint.pb'), 
        input_arrays=['superpoint/image'],
        output_arrays=['superpoint/prob', 'superpoint/descriptors'],
        input_shapes={'superpoint/image' : [None, 320, 320, 1]}
    )
    tflite_model = converter.convert()

    tflite_model_size = open('../app/src/main/assets/superpoint.tflite', 'wb').write(tflite_model)
    print('TFLite Model is %d bytes' % tflite_model_size)

if __name__ == '__main__':
    if False:
        convert_to_tflite('D:/src_code/SuperPoint/pretrained_models')
        sys.exit(0)
    ...
    weights_root_dir='D:/src_code/SuperPoint/pretrained_models'
    ...
    if False:
        ...
        graph_def=None
        with tf.io.gfile.GFile('D:/src_code/SuperPoint/pretrained_models/superpoint.pb', 'rb') as f:
    ...
    with tf.compat.v1.Session(graph=graph) as sess:
        if False:
            tf.compat.v1.saved_model.loader.load(sess,
                                   [tf.compat.v1.saved_model.tag_constants.SERVING],str(weights_dir))
            
            output_nodes = ['superpoint/descriptors', 'superpoint/prob', 'superpoint/prob_nms']
            ...
            with tf.io.gfile.GFile('D:/src_code/SuperPoint/pretrained_models/superpoint.pb', "wb") as f:
